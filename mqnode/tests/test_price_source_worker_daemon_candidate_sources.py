...
class _LifecycleManager:
    ...
    def evaluate_source(self, db, profile, now=None):
        return LifecycleDecision(
            ...
            reason='candidate WS cannot write ws_closed',
...
def test_candidate_source_daemon_polls_rest_without_starting_official_ws(source_name):
    rest_calls = []
    ws_calls = []
    ...
    worker = SourcePriceWorker(
        ...
        db=_DB(),
        settings=_settings(),
        lifecycle_manager=_LifecycleManager(source_name),
        rest_catch_up=lambda source_name, db, settings: rest_calls.append(source_name) or 2,
    ...
    assert results[0].action == 'rest_live_poll'
    assert results[0].rows_written == 2
    assert rest_calls == [source_name]
    assert ws_calls == []
